...
Type `=` then a letter to open an Excel-style list of matching functions (narrows as you type). Arrow keys move in the list; **Tab** or a **mouse click** inserts `NAME(`. **Enter** always commits what you typed (it does not take the autocomplete). A tooltip shows the signature while the `(` is open. **Esc** hides the list, or cancels the editor if the list is closed.

...
from core import Logic, USBR, USGS, Aquarius, Config, Utils
    ...
    def run(self):
        ...
        try:
            ...
            for _, dataID, _ in items:
                groupResult[dataID] = self.defaultBlanks  # Fallback blanks on error
                if db == 'AQUARIUS':
...
def customSortTable(table, col, dataDictionaryTable):
    ...
    if col not in Config.sortState:
        Config.sortState[col] = True  # Default ASC for new col
    
    else:
        Config.sortState[col] = not Config.sortState[col]  # Flip on every click
...
def executeQuery(mainWindow, queryItems, startDate, endDate, isInternal, dataDictionaryTable):
    ...
    for i, groupKey in enumerate(groups.keys()):
        ...
        if Config.debug:
            print(f"[DEBUG] Started backgroundworker {i} for group {groupKey}")
